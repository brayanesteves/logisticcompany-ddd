...
const getItems = async (req, res) => {
    try {
        await pool.query("SELECT * FROM `0_Usrs`;", (err, rows, fields) => {
            if(!err) {
                res.json(rows);
            } else {
...
const getItemByReference = async (req, res) => {
    ...
    const response = await pool.query("SELECT * FROM `0_Usrs` WHERE `Rfrnc` = $1;", [reference]);
...
const createItem = async (req, res) => {
    try {
        ...
        const addUser = await pool.query('INSERT INTO `0_Usrs`(`Usrnm`, `Psswrd`, `Rfrnc_Prsn`, `UsrTyp_Rfrnc`, `Cndtn`, `Rmvd`, `Lckd`, `DtAdmssn`, `ChckTm`) VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9)', [Usrnm, Psswrd, Rfrnc_Prsn, UsrTyp_Rfrnc, Cndtn, Rmvd, Lckd, DtAdmssn, ChckTm]);
        res.json({ 
...
const updateItemByReference = async (req, res) => {
    const reference = req.params.Rfrnc;
    const { Usrnm } = req.body;
    const response = pool.query("UPDATE `0_Usrs` SET `Usrnm` = $1 WHERE `Rfrnc` = $2;", [Usrnm, reference]);
    res.json({ 
...
const deleteItemByReference = async (req, res) => {
    const reference = req.params.Rfrnc;
    const response = pool.query("SELECT * FROM `0_Usrs` WHERE `Rfrnc` = $1;", [reference]);
